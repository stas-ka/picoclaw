...
- **RAG Intelligence Layer** — `bot_rag.py`: `classify_query()` heuristic routing (simple/factual/contextual), hardware-tier detection (FTS5_ONLY/HYBRID/FULL via psutil), Reciprocal Rank Fusion (RRF k=60) for hybrid search; PDF extraction via PyMuPDF+pdfminer fallback; per-user RAG settings (top-k, chunk size) in Profile; RAG monitoring dashboard in Admin Panel (v2026.3.32)
