# Installing Pico Bot on a New Target
...
```bash
pip3 install --break-system-packages -r /home/stas/.picoclaw/deploy/requirements.txt
```
...
Run these commands from the **Windows developer machine** (from `D:\Projects\workspace\picoclaw`).
...
plink -pw "%NEWPWD%" -batch stas@%NEWTARGET% "mkdir -p ~/.picoclaw/ssl ~/.picoclaw/notes ~/.picoclaw/calendar ~/.picoclaw/mail_creds ~/.picoclaw/error_protocols"

rem Deploy bot modules
pscp -pw "%NEWPWD%" src\bot_config.py src\bot_state.py src\bot_instance.py stas@%NEWTARGET%:/home/stas/.picoclaw/
pscp -pw "%NEWPWD%" src\bot_security.py src\bot_access.py src\bot_users.py stas@%NEWTARGET%:/home/stas/.picoclaw/
pscp -pw "%NEWPWD%" src\bot_voice.py src\bot_calendar.py src\bot_admin.py stas@%NEWTARGET%:/home/stas/.picoclaw/
pscp -pw "%NEWPWD%" src\bot_handlers.py src\bot_mail_creds.py src\bot_email.py stas@%NEWTARGET%:/home/stas/.picoclaw/
pscp -pw "%NEWPWD%" src\bot_error_protocol.py src\bot_llm.py src\bot_web.py stas@%NEWTARGET%:/home/stas/.picoclaw/
pscp -pw "%NEWPWD%" src\telegram_menu_bot.py src\gmail_digest.py src\voice_assistant.py stas@%NEWTARGET%:/home/stas/.picoclaw/
pscp -pw "%NEWPWD%" src\strings.json src\release_notes.json stas@%NEWTARGET%:/home/stas/.picoclaw/

rem Deploy Web UI templates and static assets
plink -pw "%NEWPWD%" -batch stas@%NEWTARGET% "mkdir -p ~/.picoclaw/templates ~/.picoclaw/static"
pscp -pw "%NEWPWD%" src\templates\*.html stas@%NEWTARGET%:/home/stas/.picoclaw/templates/
pscp -pw "%NEWPWD%" src\static\style.css stas@%NEWTARGET%:/home/stas/.picoclaw/static/
```
...
unzip -q /tmp/vosk.zip -d ~/.picoclaw/
mv ~/.picoclaw/vosk-model-small-ru-0.22 ~/.picoclaw/vosk-model-small-ru 2>/dev/null || true
rm /tmp/vosk.zip
...
## Step 8 — picoclaw Config

```bash
picoclaw onboard   # initialises ~/.picoclaw/config.json
```

Add an LLM model to `~/.picoclaw/config.json`:
```json
{
  "model_list": [
    {
...
This creates `~/.picoclaw/ssl/key.pem` and `~/.picoclaw/ssl/cert.pem` with proper SAN entries.

### Download cert to Windows for trust store import

```bat
pscp -pw "%NEWPWD%" stas@%NEWTARGET%:/home/stas/.picoclaw/ssl/cert.pem %NEWTARGET%.crt
```
...
```bat
certutil -addstore -f "Root" "D:\Projects\workspace\picoclaw\%NEWTARGET%.crt"
```
...
pscp -pw "%NEWPWD%" src\services\picoclaw-telegram.service stas@%NEWTARGET%:/tmp/picoclaw-telegram.service
plink -pw "%NEWPWD%" -batch stas@%NEWTARGET% "echo %NEWPWD% | sudo -S cp /tmp/picoclaw-telegram.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable picoclaw-telegram"

rem Web UI service (required for browser access)
pscp -pw "%NEWPWD%" src\services\picoclaw-web.service stas@%NEWTARGET%:/tmp/picoclaw-web.service
plink -pw "%NEWPWD%" -batch stas@%NEWTARGET% "echo %NEWPWD% | sudo -S cp /tmp/picoclaw-web.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable picoclaw-web"

rem Voice service (optional — requires RB-TalkingPI HAT)
rem pscp -pw "%NEWPWD%" src\services\picoclaw-voice.service stas@%NEWTARGET%:/tmp/picoclaw-voice.service
rem plink -pw "%NEWPWD%" -batch stas@%NEWTARGET% "echo %NEWPWD% | sudo -S cp /tmp/picoclaw-voice.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable picoclaw-voice"
```
...
plink -pw "%NEWPWD%" -batch stas@%NEWTARGET% "echo %NEWPWD% | sudo -S systemctl start picoclaw-telegram && sleep 3 && journalctl -u picoclaw-telegram -n 10 --no-pager"

rem Start Web UI
plink -pw "%NEWPWD%" -batch stas@%NEWTARGET% "echo %NEWPWD% | sudo -S systemctl start picoclaw-web && sleep 3 && journalctl -u picoclaw-web -n 10 --no-pager"
```
...
```bash
(crontab -l 2>/dev/null; echo "0 19 * * * python3 /home/stas/.picoclaw/gmail_digest.py >> /home/stas/.picoclaw/digest.log 2>&1") | crontab -
```
...
| 3 | picoclaw binary | `picoclaw version` |
| 4 | Piper TTS | `echo test \| piper --model ~/.picoclaw/ru_RU-irina-medium.onnx --output-raw > /dev/null && echo OK` |
| 5 | Source files | `ls ~/.picoclaw/telegram_menu_bot.py strings.json` |
| 6 | Voice models | `ls ~/.picoclaw/vosk-model-small-ru/ ~/.picoclaw/ru_RU-irina-medium.onnx` |
| 7 | Secrets | `cat ~/.picoclaw/bot.env \| grep BOT_TOKEN` |
| 8 | picoclaw config | `picoclaw status` |
| 9 | SSL cert | `ls ~/.picoclaw/ssl/cert.pem ~/.picoclaw/ssl/key.pem` |
| 10 | Services enabled | `systemctl is-enabled picoclaw-telegram picoclaw-web` |
| 11 | Services running | `systemctl status picoclaw-telegram picoclaw-web --no-pager` |
...
|---|---|
| `~/.picoclaw/` | All bot source files and runtime data |
| `~/.picoclaw/bot.env` | Secrets (BOT_TOKEN, ALLOWED_USER, etc.) — chmod 600 |
| `~/.picoclaw/config.json` | picoclaw LLM config (model_list, agents) |
| `~/.picoclaw/ssl/cert.pem` | TLS certificate (auto-detected by bot_web.py) |
| `~/.picoclaw/ssl/key.pem` | TLS private key |
| `~/.picoclaw/templates/` | Jinja2 HTML templates for Web UI |
| `~/.picoclaw/static/` | CSS/JS assets for Web UI |
| `~/.picoclaw/vosk-model-small-ru/` | Vosk Russian STT model (48 MB) |
| `~/.picoclaw/ru_RU-irina-medium.onnx` | Piper Russian TTS voice (66 MB) |
| `/usr/bin/picoclaw` | picoclaw Go binary |
| `/usr/local/bin/piper` | Piper TTS wrapper script |
| `/usr/local/share/piper/` | Piper binary and bundled libs |
| `/etc/systemd/system/picoclaw-*.service` | Systemd service units |
...
|---------|-----------------|------------------|
| Telegram bot | ✅ | ✅ |
| Web UI (`picoclaw-web.service`) | ❌ not installed | ✅ running |
...
rem Restart service
plink -pw "%HOSTPWD%" -batch stas@OpenClawPI "echo %HOSTPWD% | sudo -S systemctl restart picoclaw-telegram && sleep 3 && journalctl -u picoclaw-telegram -n 8 --no-pager"
```
